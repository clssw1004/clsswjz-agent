...
export class TagService {
  ...
  async findAll(userId: string, query: { accountBookId?: string }) {
    ...
    const where: any = { symbolType: TAG_TYPE };
  ...
  async create(userId: string, data: Partial<AccountSymbol>) {
    ...
    const tag = repo.create({ ...data, symbolType: TAG_TYPE, createdBy: userId, updatedBy: userId } as any);
    const saved = await repo.save(tag as any);
