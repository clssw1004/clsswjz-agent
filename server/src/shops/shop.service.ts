...
export class ShopService {
  ...
  async findAll(userId: string, query: { accountBookId?: string }) {
    const repo = await this.connMgr.getRepository(userId, AccountShop);
    const where: any = {};
  ...
  async create(userId: string, data: Partial<AccountShop>) {
    const repo = await this.connMgr.getRepository(userId, AccountShop);
    const logRepo = await this.connMgr.getRepository(userId, LogSync);
    const shop = repo.create({ ...data, createdBy: userId, updatedBy: userId } as any);
    const saved = await repo.save(shop as any);
  ...
  async update(userId: string, id: string, data: Partial<AccountShop>) {
    const repo = await this.connMgr.getRepository(userId, AccountShop);
    const logRepo = await this.connMgr.getRepository(userId, LogSync);
  ...
  async remove(userId: string, id: string) {
    const repo = await this.connMgr.getRepository(userId, AccountShop);
    const logRepo = await this.connMgr.getRepository(userId, LogSync);
    const shop = await repo.findOneBy({ id });
    await repo.delete(id);
    const log = logRepo.create({
      businessType: BusinessType.SHOP,
      operateType: OperateType.DELETE,
      parentType: 'book',
      parentId: shop?.accountBookId,
      operatorId: userId,
      operatedAt: Date.now(),
      businessId: id,
      syncState: SyncState.UNSYNCED,
