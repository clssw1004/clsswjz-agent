...
- **LogSync `parentType`/`parentId` are load-bearing, not decorative**: the main server's write gate reads `if (log.parentType === 'book') canOperateBook(log.parentId, userId)`, and GUI's `BookCULog` is the one builder that locates its row by `parentId` (`bookDao.update(parentId!, ...)`) rather than `businessId`. So every book-scoped log — including book create/update/delete — must be written as `parentType: 'book'`, `parentId: <bookId>`. Writing `'root'` silently skips the server's book permission check *and* makes book renames invisible on the phone. Conversely, `parentType: 'root'` is **required** for userShare-shared modules (the server's pull visibility only exposes `parent_type = 'root'` logs from a sharer), so do not "unify" the sentinels
